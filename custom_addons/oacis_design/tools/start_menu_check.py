#!/usr/bin/env python3
"""Verify the Oacis Start Menu (two-list layout + pin persistence).
...
def open_start_menu(page):
    ...
    page.locator(".o_navbar .o_oacis_sidebar_toggle").first.click()
    page.wait_for_timeout(800)
    return page.locator(".o_oacis_start_menu")


def main():
    ap = argparse.ArgumentParser(description="Oacis Start Menu checker (Playwright)")
    ...
    with sync_playwright() as p:
        ...
        check("sidebar rendered", page.locator(".o_oacis_sidebar").count() == 1)
        check("navbar sidebar toggle", page.locator(".o_navbar .o_oacis_sidebar_toggle").count() == 1)
        check("two-tier sidebar", page.locator(".o_oacis_sidebar_rail .o_oacis_sidebar_app").count() > 0
              and page.locator(".o_oacis_sidebar_panel .o_oacis_sidebar_group").count() > 0)
        ...
        vars_ok = page.evaluate(
            """() => {
                ...
                return { bg: s.getPropertyValue('--oacis-startmenu-bg').trim(),
                         blur: s.getPropertyValue('--oacis-startmenu-blur').trim() };
            }""",
        )
        ...
        check("full-screen overlay", page.evaluate(
            """() => { const e = document.querySelector('.o_oacis_start_menu');
                       if (!e) return false;
        ...
        # 4. Two-list layout
        favs = page.locator(".o_oacis_start_favs")
        allcol = page.locator(".o_oacis_start_all")
        ...
        all_before = allcol.locator(".o_oacis_app").count()
        check("all apps listed", all_before > 0, f"apps={all_before}")
        check("favourites empty initially", favs.locator(".o_oacis_app").count() == 0)

        # 7. Pin the first app in All Apps
        first_app = allcol.locator(".o_oacis_app").first
        app_name = first_app.locator(".o_oacis_app_name").inner_text()
        first_app.locator(".o_oacis_pin").click()
        page.wait_for_timeout(1200)
        favs_after = favs.locator(".o_oacis_app").count()
        all_after = allcol.locator(".o_oacis_app").count()
        ...
        favs_after_reload = open_start_menu(page).locator(
            ".o_oacis_start_favs .o_oacis_app_name").all_inner_texts()
        ...
        for el in page.locator(".o_oacis_start_favs .o_oacis_app").all():
            if el.locator(".o_oacis_app_name").inner_text().strip() == app_name:
                el.locator(".o_oacis_pin").click()
                page.wait_for_timeout(1200)
                break
        check("unpin returns app to all",
              favs.locator(".o_oacis_app").count() == 0
              and allcol.locator(".o_oacis_app").count() == all_before,
              f"favs={favs.locator('.o_oacis_app').count()} all={allcol.locator('.o_oacis_app').count()}")
        check("unpin cleaned DB", len(db_pins(args.db)) == 0,
              f"rows={db_pins(args.db)}")

        # 11. Search filter
        page.fill(".o_oacis_start_search", "inv")
        page.wait_for_timeout(400)
        visible_all = page.evaluate(
            """() => [...document.querySelectorAll('.o_oacis_start_all .o_oacis_app_name')]
                      .map(e => e.textContent)""")
        check("search filters results",
              len(visible_all) >= 1 and all("inv" in n.lower() for n in visible_all),
              f"results={visible_all}")
        page.fill(".o_oacis_start_search", "")
        ...
        allcol.locator(".o_oacis_app").first.click()
        ...
        page.screenshot(path="/root/odoo/odoo19/custom_addons/oacis_design/tools/start_menu_check.png", full_page=True)
        browser.close()
