...
The isolated run only loads unicore_institution_profile + its dependencies, so
the primary assertions target ``unicore.terminology.profile`` (defined in this
module, whose field labels literally carry the university terms: "Program
Label", "Student Label", ...). When the broader UniCore modules are installed
(full-suite run), the same assertions repeat on ``unicore.program`` /
``unicore.enrollment`` to prove the relabeling truly reaches unrelated modules;
...
class UniCoreTerminologyApplicationTest(TransactionCase):

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.company = cls.env.company
        # The UniCore application menu (root + Academic Structure) is restricted
        ...
        staff_group = cls.env.ref(
            'unicore_base.group_unicore_staff', raise_if_not_found=False)
        cls.staff_user = None
        if staff_group:
            cls.staff_user = cls.env['res.users'].create({
                'name': 'UniCore Staff Tester',
                'login': 'unicore_staff_tester_%s' % uuid.uuid4().hex[:8],
                'group_ids': [(6, 0, [staff_group.id])],
            })

    def _attach(self, xmlid):
        self.company.institution_profile_id = \
            self.env.ref('unicore_institution_profile.%s' % xmlid).id
    ...
    def test_01_no_profile_stays_generic(self):
        ...
        fields = self._relabel('unicore.terminology.profile',
    ...
    def test_03_training_relabels_view_arch(self):
        ...
        view = self.env.ref(
            'unicore_academic.unicore_program_search_view',
            raise_if_not_found=False)
        if not view:
            self.skipTest('unicore_academic module not installed')
        self._attach('profile_training_institute')
    ...
    def test_04_k12_relabels_fields(self):
        ...
        self.assertEqual(fields['term_department']['string'],
                         'Grade Level Label')
        fields = self._relabel('unicore.program', ['name', 'department_id'])
    ...
    def test_05_training_relabels_menus(self):
        ...
        menu = self.env.ref(
            'unicore_academic.menu_unicore_program_list',
            raise_if_not_found=False)
        if not menu:
            self.skipTest('unicore_academic module not installed')
        if not self.staff_user:
            self.skipTest('unicore_base.group_unicore_staff not available')
        self._attach('profile_training_institute')
        ...
        names = _menu_names()
        root = self.env.ref('unicore_base.menu_unicore_root',
                            raise_if_not_found=False)
        if root:
            # Staff user must see the app root menu. Its NAME differs by DB
            # ('UniCore' on fresh DBs, 'SIS' on the migrated live DB), so match
            # by record id instead of hardcoding a name.
            self.assertIn(
                root.id,
                staff_env['ir.ui.menu'].load_menus(False),
                'staff user must see the UniCore/SIS app root',
            )
    ...
    def test_06_training_relabels_action_title(self):
        ...
        action = self.env.ref(
            'unicore_academic.action_unicore_program',
            raise_if_not_found=False)
        if not action:
            self.skipTest('unicore_academic module not installed')
        self._attach('profile_training_institute')
