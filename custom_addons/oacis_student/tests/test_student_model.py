...
class OacisStudentModelTest(TransactionCase):
    ...
    def test_01_student_create_basic(self):
        student = self.env['oacis.student'].create(dict(self.student_vals))
    ...
    def test_02_student_id_sequence_unique(self):
        s1 = self.env['oacis.student'].create(dict(self.student_vals))
        vals2 = dict(self.student_vals, email='jane.doe@example.com', mobile='+919876543210')
        s2 = self.env['oacis.student'].create(vals2)
        self.assertNotEqual(
    ...
    def test_03_student_display_name(self):
        ...
        student_with_middle = self.env['oacis.student'].create(
    ...
    def test_05_student_email_is_unique_per_company(self):
        self.env['oacis.student'].create(dict(self.student_vals))
    ...
    def test_09_student_company_isolation(self):
        ...
        campus2 = self.env['oacis.campus'].create({
            'name': 'Second Campus',
            'code': 'SECCAMP',
            'company_id': company2.id,
        })
        program2 = self.env['oacis.program'].create({
            'name': 'Second Program',
            'code': 'SECPROG',
            'program_type': 'undergraduate',
        ...
        students_company1 = self.env['oacis.student'].search([('company_id', '=', self.company.id)])
    ...
    def test_10_student_min_age_validation(self):
        ...
        with self.assertRaises(ValidationError):
            self.env['oacis.student'].create(too_young)


@odoo.tests.tagged('oacis', 'unit')
class OacisCurriculumDependencyCheck(TransactionCase):
