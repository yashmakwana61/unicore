...
docker compose run --rm web odoo -c /etc/odoo/odoo.conf -d postgres -i "$INSTALL_ORDER" --stop-after-init

echo "Initialization complete. You can now run: docker compose up -d web"
